...
class Solution(object):
    def splitArray(self, nums, m):
        ...
        """
        l = len(nums)
        sum = [[ 0 if i>0 else nums[j] for j in range(l)] for i in range(l)]
...
#         print indice
    ...
    def integerBreak(self, m, n):   
        """
        :type m: int integer to be broken
        :type n: int length 
        :rtype: int
        """
        ret = []
        self._integerBreak(m, n, n, [], ret)
        return ret
            
        
    def _integerBreak(self, m, n, l, list, ret):     
        if(m == 0 or n == 0):
            return 
        if n == 1:
            list.append(m)
        ...
        for i in range(1, m-n+2):
            list.append(i)
            self._integerBreak(m-i, n-1, l, list, ret)
            list.pop()
            
        
s = Solution()
# print s.integerBreak(5, 3)
...
m = 9
print s.integerBreak(len(nums), m)
print len(nums)
# r = s.splitArray(nums, m)
# print r